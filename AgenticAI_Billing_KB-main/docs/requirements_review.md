...
| P1 | Deployment and observability, page 5. Docker now includes OCR tools and Compose runs the health API plus dashboard with persistent data/runtime paths. An ephemeral CloudFormation/SSM deployment and cleanup runbook exist, but have not been executed against the ISEG account. Structured application metrics/tracing remain absent. | Run and delete the temporary stack with the assigned ISEG role; preserve redacted deployment evidence. Add structured stage logs, correlation IDs, duration/error metrics, and redacted traces. Demonstrate one traced invoice from upload to review. |
