...
An experimental coded-agent Plan B coordinates classification, provider memory, Gemini or OpenAI extraction, stronger validation, and review routing; stores a step trace; compares its fields with the complete July OCR + direct LLM Plan A; and captures a human accuracy verdict. The strict five-distinct-prior-approvals rule and stronger consistency checks currently belong to Plan B, so the P0 production gaps below remain until A/B evidence supports promotion.

| Priority | Requirement and current evidence | Concrete next step and completion evidence |
| --- | --- | --- |
| P0 | Validated history and approval, pages 3–4. Preprocessed CSV rows are now stored as `observed_invoices` with `review_status=unreviewed`; they do not enter `previously_validated_invoices`. The automatic-approval gate counts five distinct prior records with `review_decision=human_approved`. | Add immutable invoice/extraction version IDs and test four versus five approved examples, repeated uploads, corrections, superseded versions, and rejection. Persist automatic decisions with reasons if automation is enabled. |
...
| P1 | Persistent SQL/NoSQL knowledge base, page 4. Provider memory now uses transactional SQLite tables for providers, observed invoices, validated invoices, known layouts, review feedback and validation history, with a derived FAISS/LlamaIndex index. Local and AWS containers share `/app/runtime/knowledge_base.sqlite3`. | Add immutable extraction versions, per-field confidence/provenance and OCR anomalies. Demonstrate concurrent review behavior and a restart on the temporary AWS host. |
...
| P1 | Deployment and observability, page 5. Docker includes OCR tools and Compose runs the readiness API, dashboard, and Gradio lab with persistent data/runtime paths. The manual AWS flow verifies and transfers the exact locally tested image, checks its OCR fingerprint, and has a cleanup runbook, but it has not been executed against the assigned AWS account. Structured application metrics/tracing remain incomplete. | Run and delete the temporary stack with the assigned AWS role; preserve redacted deployment evidence. Add correlation IDs and aggregated duration/error metrics. Demonstrate one traced invoice from upload to review. |
