...
flowchart TD
    ...
    subgraph LocalMemory["Ignored local runtime memory"]
        Memory["runtime/knowledge_base.sqlite3 (ignored local SQL memory)"]
    end
...
- Keep provider memory in `runtime/knowledge_base.sqlite3`; it may contain reviewer feedback and should remain ignored.
